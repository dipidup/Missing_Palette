...
def fetch_sheet(url):
    ...
    Soft-fails (exits 0) if the URL is empty so the workflow can still run
    before the secret is configured. Set GALLERY_SHEET_CSV_URL to enable.
    """
    if not url:
        print("note: GALLERY_SHEET_CSV_URL not set. Skipping sheet-driven "
              "gallery rebuild. (Set the env var / GitHub secret to enable.)")
        sys.exit(0)
...
def render_card(row, medium_label):
    ...
    # Modern-format siblings sit next to the source jpg/png.
    # scripts/convert_gallery_images.py generates them; if they don't
    # exist yet the <img> fallback still works.
    stem, _, _ = filename.rpartition('.')
    avif_src = f'img/gallery/{stem}.avif' if stem else src
    webp_src = f'img/gallery/{stem}.webp' if stem else src
    ...
    card = (
        ...
        f'                        <img src="{src}" alt="{title_raw}" loading="lazy">\n'
        f'                    </picture>\n'
        f'                </div>\n'
